...
rcxDict.prototype = {
    ...
    fileRead : function(url, charset) {
        ...
        if (url == "data/dict.dat") {
            return myData.join('\n');
        } else if (url == "data/dict.idx") {
            return myDataIdx.join('\n');
        } else if (url == "data/kanji.dat") {
            return myKanji.join('\n');
        } else if (url == "data/names.dat") {
            return myName.join('\n');
        } else if (url == "data/names.idx") {
            return myNameIdx.join('\n');
        }
        return null;
    ...
    loadNames : function() {
         ...
         * rcxNamesDict.datCharset); this.nameIndex =
         * this.fileRead(rcxNamesDict.idxURI, rcxNamesDict.idxCharset);
         */
        this.nameDict = this.fileRead(("data/names.dat"));
        this.nameIndex = this.fileRead(("data/names.idx"));
    },
    ...
    loadDictionary : function() {
         ...
         * rcxWordDict.datCharset); this.wordIndex =
         * this.fileRead(rcxWordDict.idxURI, rcxWordDict.idxCharset);
         */
    ...
    /*
     * test_kanji: function() { var a = this.kanjiData.split('\n');
     *
     * alert('begin test. a.length=' + a.length); var start = (new
     * Date()).getTime(); for (var i = 0; i < a.length; ++i) { if
     * (!this.kanjiSearch(a[i].charAt(0))) { alert('error @' + i + ': ' + a[i]);
     * return; } } alert('time = ' + ((new Date()).getTime() - start)); },
     */

    /*
     * test_index: function() { var ixF =
     * this.fileRead('chrome://rikaichan/content/dict.idx', 'EUC-JP'); var ixA =
     * ixF.split('\n');
     *
     * while ((ixA.length > 0) && (ixA[ixA.length - 1].length == 0)) ixA.pop();
     *  // alert('length=' + ixA.length + ' / ' + ixF.length); if (0) { var
     * timeA = (new Date()).getTime(); for (var i = ixA.length - 1; i >= 0; --i) {
     * if ((i & 0xFF) == 0) window.status = 'A: ' + i; var s = ixA[i]; var r =
     * this.binSearchX(ixA, s.substr(0, s.indexOf(',') + 1)); if ((r == -1) ||
     * (ixA[r] != s)) { alert('A failed: ' + s); return; } } } timeA = ((new
     * Date()).getTime() - timeA) / 1000;
     *
     *
     * var timeF = (new Date()).getTime(); for (var i = ixA.length - 1; i >= 0;
     * --i) { if ((i & 0xFF) == 0) window.status = 'F: ' + i; var s = ixA[i];
     * var r = this.find(ixF, s.substr(0, s.indexOf(',') + 1)); if (r != s) {
     * alert('F failed: ' + s); return; } } timeF = ((new Date()).getTime() -
     * timeF) / 1000;
     *
     * var timeX = (new Date()).getTime(); if (0) { for (var i = ixA.length - 1;
     * i >= 0; --i) { if ((i & 0xFF) == 0) window.status = 'X: ' + i; var s =
     * ixA[i];
     *
     * var w = s.substr(0, s.indexOf(',') + 1); var j = 0; r = ''; if
     * (ixF.substr(0, w.length) == w) { r = ixF.substr(0, ixF.indexOf('\n')); }
     * else { w = '\n' + w; j = ixF.indexOf(w); if (j != -1) r = ixF.substring(j +
     * 1, ixF.indexOf('\n', j + 1)); }
     *
     * if (r != s) { alert('X failed:\n[' + s + ']\n[' + r + ']'); return; } } }
                ...
                if (g.flen <= wordLen) {
                    ...
                    for (k = 0; k < g.length; ++k) {
                        ...
                        if ((type & rule.type) && (end == rule.from)) {
                            var newWord = word.substr(0, word.length
                                    - rule.from.length)
                                    + rule.to;
                            if (newWord.length <= 1)
                                continue;
    ...
    ch : [ 0x3092, 0x3041, 0x3043, 0x3045, 0x3047, 0x3049, 0x3083, 0x3085,
    ...
    cv : [ 0x30F4, 0xFF74, 0xFF75, 0x304C, 0x304E, 0x3050, 0x3052, 0x3054,
    ...
    wordSearch : function(word, doNames, max) {
        ...
        for (i = 0; i < word.length; ++i) {
            ...
            trueLen[r.length] = i + 1; // need to keep real length because of
                                        // the half-width semi/voiced conversion
            p = v;
        }
        word = r;
    ...
    numList : [
    /*
     * 'C', 'Classical Radical', 'DR', 'Father Joseph De Roo Index', 'DO', 'P.G.
     * O\'Neill Index', 'O', 'P.G. O\'Neill Japanese Names Index', 'Q', 'Four
     * Corner Code', 'MN', 'Morohashi Daikanwajiten Index', 'MP', 'Morohashi
     * Daikanwajiten Volume/Page', 'K', 'Gakken Kanji Dictionary Index', 'W',
     * 'Korean Reading',
     */
    'H', 'Halpern', 'L', 'Heisig', 'E', 'Henshall', 'DK',
            'Kanji Learners Dictionary', 'N', 'Nelson', 'V', 'New Nelson', 'Y',
            'PinYin', 'P', 'Skip Pattern', 'IN', 'Tuttle Kanji &amp; Kana',
            'I', 'Tuttle Kanji Dictionary', 'U', 'Unicode' ],
    ...
    makeHtml : function(entry) {
        ...
        if (entry.kanji) {
            ...
            box = '<table class="k-abox-tb"><tr>'
                    + '<td class="k-abox-r">Bộ<br/>'
                    + this.radData[bn].charAt(0) + ' ' + (bn + 1) + '</td>'
                    + '<td class="k-abox-g">' + k + '</td>' + '</tr><tr>'
                    + '<td class="k-abox-f">Tần suất<br/>'
                    + (entry.misc['F'] ? entry.misc['F'] : '-') + '</td>'
                    + '<td class="k-abox-s">Nét<br/>' + entry.misc['S']
                    + '</td>' + '</tr></table>';
            ...
            // nums += '<tr><td' + c + '>' + this.numList[i*2 + 1] + '</td><td'
            // + c + '>' + (s ? s : '-') + '</td></tr>';
            // }
            // }
            if (nums.length)
        ...
        if (entry.names) {
            ...
            b
                    .push('<div class="w-title">Names Dictionary</div><table class="w-na-tb"><tr><td>');
            for (i = 0; i < entry.data.length; ++i) {
                ...
                // the next two lines re-process the entries that contain
                // separate search key and spelling due to mixed
                // hiragana/katakana spelling
                e3 = e[3].match(/^(.+?)\s+(?:\[(.*?)\])?\s*\/(.+)\//);
                if (e3)
                    e = e3;
                ...
                if (e[2])
                    c.push('<span class="w-kanji">' + e[1]
                            + '</span> &#32; <span class="w-kana">' + e[2]
                            + '</span><br/> ');
                else
                ...
                console.log('e[2]: ' + e[2]);
                console.log('e[3]: ' + e[3]);
                t = '<span class="w-def">' + s.replace(/\//g, '; ')
                        + '</span><br/>';
            }
            c.push(t);
            if (c.length > 4) {
            ...
            for (i = 0; i < entry.data.length; ++i) {
                ...
                /*
                 * e[1] = kanji/kana e[2] = kana e[3] = definition
                 */
                ...
                if (entry.data[i][1])
                    b.push(' <span class="w-conj">(' + entry.data[i][1]
                            + ')</span>');
        ...
        if (entry.kanji) {
            ...
            for (i = 0; i < this.numList.length; i += 2) {
                ...
                if (/* this.config.kdisp[e] */1 == 1) {
                    j = entry.misc[e];
                    b.push(this.numList[i + 1].replace('&amp;', '&') + '\t'
                            + (j ? j : '-') + '\n');
                }
            }
        } else {
