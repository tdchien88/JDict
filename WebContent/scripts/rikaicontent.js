...
var sendMessage = function(request, sender, response) {
    switch (request.type) {
    case 'enable?':
        console.log('enable?');
    ...
    case 'xsearch':
        // console.log('xsearch');
        var e = rcxMain.wordSearch(request.text, rcxMain.config.doName, 5);
        // console.log(e);
        /*
         * var xxx = '' $.each(e.data, function( index, value ) { xxx +=
         * value.toString() +'<br /><br />' }); $('#mydict').html(xxx);
         */
        ...
        break;
    /*
     * case 'nextDict': console.log('nextDict'); rcxMain.nextDict(); break;
     */
    case 'resetDict':
    ...
    case 'translate':
        ...
        var e = rcxMain.translate(request.title);
        response(e);
        break;
    case 'makehtml':
        console.log('makehtml');
        var html = rcxMain.makeHtml(request.entry);
...
var rcxContent = {
    ...
    enableTab : function() {
        if (window.rikaichan == null) {
            window.rikaichan = {};
            window.rikaichan.config = rcxMain.config;
    ...
    showPopup : function(text, elem, x, y, looseWidth) {
        ...
        if (!popup) {
            ...
            css.setAttribute('href', 'css/popup-blue.css');
            css.setAttribute('id', 'rikaichan-css');
            topdoc.getElementsByTagName('head')[0].appendChild(css);

            popup = topdoc.createElementNS('http://www.w3.org/1999/xhtml',
                    'div');
            ...
            /*
             * if (this.cfg.resizedoc) { if ((topdoc.body.clientHeight < 1024) &&
             * (topdoc.body.style.minHeight == '')) {
        ...
        if (rcxContent.getContentType(topdoc) == 'text/plain') {
            ...
            df.appendChild(document.createElementNS(
        ...
        if (elem) {
            ...
            // FIXME: This probably doesn't actually work
                ...
                /*
                 * // in some cases (ex: google.co.jp), ebo doesn't add the
                 * width of the scroller (?), so use SELECT's width const epbo =
                 * elem.ownerDocument.getBoxObjectFor(elem.parentNode);
                 *
                 * const ebo = elem.ownerDocument.getBoxObjectFor(elem); x =
                 * ebo.screenX - bbo.screenX; y = ebo.screenY - bbo.screenY;
                 *
                 * if (x > (window.innerWidth - (x + epbo.width))) { x = (x -
                 * popup.offsetWidth - 5); if (x < 0) x = 0; } else { x +=
                 * epbo.width + 5; }
                 */
            } else {
                // x -= bbo.screenX;
                ...
                if ((y + v + pH) > window.innerHeight) {
                    ...
                    if (t >= 0) {
                        ...
                        // if can't go up, still go down to prevent blocking
                        // cursor
                        y += v;
                    }
                } else
    ...
    clearHi : function() {
        ...
        if ((!sel.toString()) || (tdata.selText == sel.toString())) {
            ...
            // which we just cleared and now we're going to jump back to where
            // we were
            // the cursor was before our lookup
            // if oldCaret is less than 0 it means we clicked outside the box
            // and shouldn't
            // come back
    ...
    configPage : function() {
        window.openDialog('chrome://rikaichan/content/prefs.xul', '',
                'chrome,centerscreen');
    },
    ...
    _onKeyDown : function(ev) {
        ...
        if (window.rikaichan.config.showOnKey !== ""
    ...
    onKeyUp : function(ev) {
        if (rcxContent.keysDown[ev.keyCode])
            rcxContent.keysDown[ev.keyCode] = 0;
    },
    ...
    isInline : function(node) {
        return this.inlineNames.hasOwnProperty(node.nodeName)
                ...
                (document.nodeType == Node.ELEMENT_NODE && (document.defaultView
                        .getComputedStyle(node, null).getPropertyValue(
                                'display') == 'inline' || document.defaultView
                        .getComputedStyle(node, null).getPropertyValue(
    ...
    getTextFromRange : function(rangeParent, offset, selEndList, maxLength) {
        if (rangeParent.nodeName == 'TEXTAREA'
            ...
            // TODO - chien dont do anything if input
            return;
            var endIndex = Math
                    .min(rangeParent.data.length, offset + maxLength);
    ...
    show : function(tdata, dictOption) {
        ...
        //
        if ((isNaN(u))
                || ((u != 0x25CB) && ((u < 0x3001) || (u > 0x30FF))
                        && ((u < 0x3400) || (u > 0x9FFF))
                        && ((u < 0xF900) || (u > 0xFAFF)) && ((u < 0xFF10) || (u > 0xFF9D)))) {
            this.clearHi();
            this.hidePopup();
            return -2;
        }
    ...
    processEntry : function(e) {
        ...
        if ((rp)
                && ((tdata.config.highlight == 'true' && !this.mDown && !('form' in tdata.prevTarget)) || (('form' in tdata.prevTarget) && tdata.config.textboxhl == 'true'))) {
            ...
            rcxContent.highlightMatch(doc, rp, ro, e.matchLen, selEndList,
                    tdata);
    ...
    processHtml : function(html) {
        tdata = window.rikaichan;
        rcxContent.showPopup(html, tdata.prevTarget, tdata.popX, tdata.popY,
                false);
        return 1;
    },
    ...
    /*
     * inRange: function (event) { var selection = event.view.getSelection(); if
     * ((selection.rangeCount === 0) || (!event.rangeParent)) return false; var
     * newRange = event.view.document.createRange();
     * newRange.setStart(event.rangeParent, event.rangeOffset);
     * newRange.setEnd(event.rangeParent, event.rangeOffset);
     *
     * var curRange = selection.getRangeAt(0); if
     * (newRange.compareBoundaryPoints(Range.START_TO_START, curRange) > -1 &&
     * newRange.compareBoundaryPoints(Range.END_TO_END, curRange) < 0) return
    ...
    tryUpdatePopup : function(ev) {
        ...
        try {
             ...
             * total size: " + (rp.data?rp.data.length:"") + " target: " +
             * ev.target.nodeName + " parentparent: " + rp.parentNode.nodeName);
             */

            // This is to account for bugs in caretRangeFromPoint
            // It includes the fact that it returns text nodes over non text
            // nodes
            // and also the fact that it miss the first character of inline
            // nodes.
        ...
        /*
         * if ((rp) && (rp.nodeType != Node.TEXT_NODE) && !('form' in rp)) { rp =
         * null; ro = -1; }
         */
        ...
        if ((rp) && (rp.data) && (ro < rp.data.length)) {
            ...
            tdata.timer = setTimeout(function(rangeNode, rangeOffset) {
                if (!window.rikaichan
                ...
                rcxContent.show(tdata,
                        rcxContent.forceKanji ? rcxContent.forceKanji
        ...
        if (tdata.title) {
            tdata.popX = ev.clientX;
            tdata.popY = ev.clientY;
            tdata.timer = setTimeout(function(tdata, title) {
